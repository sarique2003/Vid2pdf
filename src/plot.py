...
def plot_timestamps_vs_pixel_change(stats):
    ...
    for frame_num in stats:
        ...
        simple_timestamp[timestamp_seconds] = [
            min(simple_timestamp[timestamp_seconds]
                [0], num_pixels_changed),  # min
            max(simple_timestamp[timestamp_seconds]
                [1], num_pixels_changed),  # max
...
def save_selected_frames(selected_frames):
    ...
    for frame_num in selected_frames:
        ...
        cv2.imwrite(
            "plot-output/{}_{}_cur_frame.jpeg".format(
                timestamp, pixel_changes), frame
        )
        cv2.imwrite(
            "plot-output/{}_{}_next_frame.jpeg".format(
                timestamp, pixel_changes),
            next_frame,
        )
        cv2.imwrite(
            "plot-output/{}_{}_mask_frame.jpeg".format(
                timestamp, pixel_changes), mask
        )
